...
# build 'netscale-darwin-amd64.tgz'
mkdir -p artifacts
FILENAME="$(pwd)/artifacts/netscale-darwin-amd64.tgz"
PKGNAME="$(pwd)/artifacts/netscale-amd64.pkg"
...
BUNDLE_ID="com.khulnasoft.netscale"
SEC_DUP_MSG="security: SecKeychainItemImport: The specified item already exists in the keychain."
export PATH="$PATH:/usr/local/bin"
mkdir -p ../src/github.com/khulnasoft/    
cp -r . ../src/github.com/khulnasoft/netscale
cd ../src/github.com/khulnasoft/netscale 
GOCACHE="$PWD/../../../../" GOPATH="$PWD/../../../../" CGO_ENABLED=1 make netscale
...
# copy netscale into the build directory
cp ${BINARY_NAME} "${TARGET_DIRECTORY}/contents/${PRODUCT}"

# compress netscale into a tar and gzipped file
tar czf "$FILENAME" "${BINARY_NAME}"

# build the installer package
if [[ ! -z "$PKG_SIGN_NAME" ]]; then
  pkgbuild --identifier com.khulnasoft.${PRODUCT} \
      --version ${VERSION} \
