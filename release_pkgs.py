...
R2_ASSET_URL = 'https://demo-r2-worker.khulnasoft-tunnel.workers.dev/'
...
class PkgUploader:
    ...
    def upload_pkg_to_r2(self, filename, upload_file_path):
        endpoint_url = f"https://{self.account_id}.r2.khulnasoftstorage.com"
...
class PkgCreator:
    ...
    """
        ...
        [netscale-stable]
        name=netscale-stable
        baseurl=https://pkg.khulnasoft.com/netscale/rpm
        enabled=1
        type=rpm
        gpgcheck=1
        gpgkey=https://pkg.khulnasoft.com/khulnasoft-main.gpg
    """
...
""" 
    1. looks into a built_artifacts folder for netscale debs
    2. creates Packages.gz, InRelease (signed) files
    3. uploads them to Khulnasoft R2 
    ...
    release_version: is the netscale release version. Only provide this if you want a permanent backup.
"""
...
def create_deb_packaging(pkg_creator, pkg_uploader, releases, gpg_key_id, binary_name, archs, package_component,
    ...
    for release in releases:
        for arch in archs:
            ...
            pkg_creator.create_deb_pkgs(release, f"./built_artifacts/netscale-linux-{arch}.deb")
