...
import (
	...
	"github.com/khulnasoft/netscale/datagramsession"
	"github.com/khulnasoft/netscale/ingress"
	"github.com/khulnasoft/netscale/management"
	"github.com/khulnasoft/netscale/packet"
...
func (q *QUICConnection) Serve(ctx context.Context) error {
	...
	// In the future, if netscale can autonomously push traffic to the edge, we have to make sure the control
...
func (q *QUICConnection) runStream(quicStream quic.Stream) {
	...
	// we are going to fuse readers/writers from stream <- netscale -> origin, and we want to guarantee that
...
func (s *streamReadWriteAcker) AckConnection(tracePropagation string) error {
	...
	if tracePropagation != "" {
		metadata = append(metadata, quicpogs.Metadata{
			Key: tracing.CanonicalNetscaleTracingHeader,
