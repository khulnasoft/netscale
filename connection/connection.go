...
import (
	...
	"github.com/khulnasoft/netscale/tunnelrpc/pogs"
	"github.com/khulnasoft/netscale/websocket"
)

const (
	lbProbeUserAgentPrefix = "Mozilla/5.0 (compatible; Khulnasoft-Traffic-Manager/1.0; +https://www.khulnasoft.com/traffic-manager/;"
...
func (h *HTTPResponseReadWriteAcker) AckConnection(tracePropagation string) error {
	...
	if tracePropagation != "" {
		resp.Header.Add(tracing.CanonicalNetscaleTracingHeader, tracePropagation)
	}

	return h.w.WriteRespHeaders(resp.StatusCode, resp.Header)
}

// localProxyConnection emulates an incoming connection to netscale as a net.Conn.
