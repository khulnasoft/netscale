...
const (
	baseStoreURL  = "https://login.khulnasoftaccess.org/"
...
func RunTransfer(transferURL *url.URL, appAUD, resourceName, key, value string, shouldEncrypt bool, useHostOnly bool, log *zerolog.Logger) ([]byte, error) {
	encrypterClient, err := NewEncrypter("netscale_priv.pem", "netscale_pub.pem")
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Please open the following URL and log in with your Khulnasoft account:\n\n%s\n\nLeave netscale running to download the %s automatically.\n", requestURL, resourceName)
	} else {
