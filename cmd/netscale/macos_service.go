...
const (
	launchdIdentifier = "com.khulnasoft.netscale"
)

func runApp(app *cli.App, graceShutdownC chan struct{}) {
	app.Commands = append(app.Commands, &cli.Command{
		Name:  "service",
		Usage: "Manages the netscale launch agent",
		Subcommands: []*cli.Command{
			{
				Name:   "install",
				Usage:  "Install netscale as an user launch agent",
				Action: cliutil.ConfiguredAction(installLaunchd),
			},
			{
				Name:   "uninstall",
				Usage:  "Uninstall the netscale launch agent",
...
func installLaunchd(c *cli.Context) error {
	...
	if isRootUser() {
		log.Info().Msg("Installing netscale client as a system launch daemon. " +
			"netscale client will run at boot")
	} else {
		log.Info().Msg("Installing netscale client as an user launch agent. " +
			"Note that netscale client will only run when the user is logged in. " +
			"If you want to run netscale client at boot, install with root permission. " +
	...
	if err == nil {
		log.Info().Msg("MacOS service for netscale installed successfully")
	}
	return err
}
...
func uninstallLaunchd(c *cli.Context) error {
	...
	if isRootUser() {
		log.Info().Msg("Uninstalling netscale as a system launch daemon")
	} else {
		log.Info().Msg("Uninstalling netscale as a user launch agent")
	}
	installPath, err := installPath()
	if err != nil {
		return errors.Wrap(err, "error determining install path")
	...
	if err == nil {
		log.Info().Msg("Launchd for netscale was uninstalled successfully")
	}
	return err
}
