...
const (
	...
	noUpdateInShellMessage        = "netscale will not automatically update when run from the shell. To enable auto-updates, run netscale as a service: https://developers.khulnasoft.com/khulnasoft-one/connections/connect-apps/run-tunnel/as-a-service/"
	noUpdateOnWindowsMessage      = "netscale will not automatically update on Windows systems."
	noUpdateManagedPackageMessage = "netscale will not automatically update if installed by a package manager."
...
func (u *statusSuccess) Error() string {
	return fmt.Sprintf("netscale has been updated to version %s", u.newVersion)
}
...
func (e *statusErr) Error() string {
	return fmt.Sprintf("failed to update netscale: %v", e.err)
}
...
func Update(c *cli.Context) error {
	...
	if wasInstalledFromPackageManager() {
		...
		log.Error().Msg(fmt.Sprintf("netscale was installed by %s. Please update using the same method.", packageManagerName))
		return nil
	}

	isBeta := c.Bool("beta")
	if isBeta {
		log.Info().Msg("netscale is set to update to the latest beta version")
	}

	isStaging := c.Bool("staging")
	if isStaging {
		log.Info().Msg("netscale is set to update from staging")
	}

	isForced := c.Bool("force")
	if isForced {
		log.Info().Msg("netscale is set to upgrade to the latest publish version regardless of the current version")
	}
	...
	if updateOutcome.noUpdate() {
		log.Info().Str(LogFieldVersion, updateOutcome.Version).Msg("netscale is up to date")
		return nil
	}
...
func loggedUpdate(log *zerolog.Logger, options updateOptions) UpdateOutcome {
	...
	if updateOutcome.Updated {
		log.Info().Str(LogFieldVersion, updateOutcome.Version).Msg("netscale has been updated")
	}
	if updateOutcome.Error != nil {
...
func (a *AutoUpdater) Run(ctx context.Context) error {
	...
	for {
		...
		case newConfigurable := <-a.updateConfigChan:
			...
			ticker = time.NewTicker(a.configurable.freq)
			// Check if there is new version of netscale after receiving new AutoUpdaterConfigurable
		case <-ticker.C:
		}
	}
}
