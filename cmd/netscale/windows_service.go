...
const (
	windowsServiceName        = "Netscale"
	windowsServiceDescription = "Netscale agent"
	windowsServiceUrl         = "https://developers.khulnasoft.com/khulnasoft-one/connections/connect-apps/run-tunnel/as-a-service/windows/"
...
func runApp(app *cli.App, graceShutdownC chan struct{}) {
	app.Commands = append(app.Commands, &cli.Command{
		Name:  "service",
		Usage: "Manages the netscale Windows service",
		Subcommands: []*cli.Command{
			{
				Name:   "install",
				Usage:  "Install netscale as a Windows service",
	...
	// like running netscale in a GCP startup script, for which
...
func (s *windowsService) Execute(serviceArgs []string, r <-chan svc.ChangeRequest, statusChan chan<- svc.Status) (ssec bool, errno uint32) {
	...
	for {
		...
		case c := <-r:
			...
			case svc.Stop, svc.Shutdown:
				if s.graceShutdownC != nil {
					...
					elog.Info(1, "netscale starting graceful shutdown")
				...
				elog.Info(1, "netscale terminating immediately")
				statusChan <- svc.Status{State: svc.StopPending}
				return false, 0
			default:
		...
		case err := <-errC:
			if err != nil {
				elog.Error(1, fmt.Sprintf("netscale terminated with error %v", err))
				ssec = true
				errno = 1
			} else {
				elog.Info(1, "netscale terminated without error")
				errno = 0
			}
			return
		}
...
func installWindowsService(c *cli.Context) error {
	...
	zeroLogger.Info().Msg("Installing netscale Windows service")
	...
	defer s.Close()
	log.Info().Msg("netscale agent service is installed")
	...
	err = s.Start()
	if err == nil {
		log.Info().Msg("Agent service for netscale installed successfully")
	}
	return err
}
...
func uninstallWindowsService(c *cli.Context) error {
	...
	log.Info().Msg("Uninstalling netscale agent service")
	m, err := mgr.Connect()
	if err != nil {
		return errors.Wrap(err, "Cannot establish a connection to the service control manager")
	}
	...
	if status, err := s.Query(); err == nil && status.State == svc.Running {
		log.Info().Msg("Stopping netscale agent service")
		if _, err := s.Control(svc.Stop); err != nil {
			log.Info().Err(err).Msg("Failed to stop netscale agent service, you may need to stop it manually to complete uninstall.")
		}
	}
	...
	log.Info().Msg("Agent service for netscale was uninstalled successfully")
