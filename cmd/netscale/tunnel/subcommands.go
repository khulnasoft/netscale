...
import (
	...
	"github.com/khulnasoft/netscale/cfapi"
...
var (
	...
	selectProtocolFlag = altsrc.NewStringFlag(&cli.StringFlag{
		...
		Aliases: []string{"p"},
		Usage:   fmt.Sprintf("Protocol implementation to connect with Khulnasoft's edge network. %s", connection.AvailableProtocolFlagMessage),
	...
	cleanupClientFlag = &cli.StringFlag{
		Name:    "connector-id",
		Aliases: []string{"c"},
		Usage:   `Constraints the cleanup to stop the connections of a single Connector (by its ID). You can find the various Connectors (and their IDs) currently connected to your tunnel via 'netscale tunnel info <name>'.`,
	...
	icmpv4SrcFlag = &cli.StringFlag{
		Name:    "icmpv4-src",
		Usage:   "Source address to send/receive ICMPv4 messages. If not provided netscale will dial a local address to determine the source IP or fallback to 0.0.0.0.",
		EnvVars: []string{"TUNNEL_ICMPV4_SRC"},
	}
	icmpv6SrcFlag = &cli.StringFlag{
		Name:    "icmpv6-src",
		Usage:   "Source address and the interface name to send/receive ICMPv6 messages. If not provided netscale will dial a local address to determine the source IP or fallback to ::.",
...
func buildCreateCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText: "netscale tunnel [tunnel command options] create [subcommand options] NAME",
		Description: `Creates a tunnel, registers it with Khulnasoft edge and generates credential file used to run this tunnel.
  Use "netscale tunnel route" subcommand to map a DNS name to this tunnel and "netscale tunnel run" to start the connection.

  For example, to create a tunnel named 'my-tunnel' run:

  $ netscale tunnel create my-tunnel`,
...
func createCommand(c *cli.Context) error {
	...
	if c.NArg() != 1 {
		return cliutil.UsageError(`"netscale tunnel create" requires exactly 1 argument, the name of tunnel to create.`)
	}
	name := c.Args().First()
...
func buildListCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText:   "netscale tunnel [tunnel command options] list [subcommand options]",
		Description: "netscale tunnel list will display all active tunnels, their created time and associated connections. Use -d flag to include deleted tunnels. See the list of options to filter the list",
		Flags: []cli.Flag{
			outputFormatFlag,
			showDeletedFlag,
			listNameFlag,
...
func listCommand(c *cli.Context) error {
	...
	if len(tunnels) > 0 {
		...
		fmt.Println("No tunnels were found for the given filter flags. You can use 'netscale tunnel create' to create a tunnel.")
	}
...
func formatAndPrintTunnelList(tunnels []*cfapi.Tunnel, showRecentlyDisconnected bool) {
	...
	_, _ = fmt.Fprintln(writer, "You can obtain more detailed information for each tunnel with `netscale tunnel info <name/uuid>`")
...
func buildInfoCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText:   "netscale tunnel [tunnel command options] info [subcommand options] [TUNNEL]",
		Description: "netscale tunnel info displays details about the active connectors for a given tunnel (identified by name or uuid).",
		Flags: []cli.Flag{
			outputFormatFlag,
			showRecentlyDisconnected,
			sortInfoByFlag,
...
func tunnelInfo(c *cli.Context) error {
	...
	if c.NArg() != 1 {
		return cliutil.UsageError(`"netscale tunnel info" accepts exactly one argument, the ID or name of the tunnel to get info about.`)
	}
...
func buildDeleteCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText:          "netscale tunnel [tunnel command options] delete [subcommand options] TUNNEL",
		Description:        "netscale tunnel delete will delete tunnels with the given tunnel UUIDs or names. A tunnel cannot be deleted if it has active connections. To delete the tunnel unconditionally, use -f flag.",
...
func deleteCommand(c *cli.Context) error {
	...
	if c.NArg() < 1 {
		return cliutil.UsageError(`"netscale tunnel delete" requires at least 1 argument, the ID or name of the tunnel to delete.`)
	}
...
func buildRunCommand() *cli.Command {
	...
	return &cli.Command{
		...
		UsageText: "netscale tunnel [tunnel command options] run [subcommand options] [TUNNEL]",
		Description: `Runs the tunnel identified by name or UUID, creating highly available connections
  between your server and the Khulnasoft edge. You can provide name or UUID of tunnel to run either as the
  last command line argument or in the configuration file using "tunnel: TUNNEL".

  This command requires the tunnel credentials file created when "netscale tunnel create" was run,
  however it does not need access to cert.pem from "netscale login" if you identify the tunnel by UUID.
  If you experience other problems running the tunnel, "netscale tunnel cleanup" may help by removing
...
func runCommand(c *cli.Context) error {
	...
	if c.NArg() > 1 {
		return cliutil.UsageError(`"netscale tunnel run" accepts only one argument, the ID or name of the tunnel to run.`)
	}

	if c.String("hostname") != "" {
		sc.log.Warn().Msg("The property `hostname` in your configuration is ignored because you configured a Named Tunnel " +
			"in the property `tunnel` to run. Make sure to provision the routing (e.g. via `netscale tunnel route dns/lb`) or else " +
	...
	if tokenStr := c.String(TunnelTokenFlag); tokenStr != "" {
		...
		if tunnelRef == "" {
			...
			if tunnelRef == "" {
				return cliutil.UsageError(`"netscale tunnel run" requires the ID or name of the tunnel to run as the last command line argument or in the configuration file.`)
			}
		}
...
func buildCleanupCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText:          "netscale tunnel [tunnel command options] cleanup [subcommand options] TUNNEL",
...
func cleanupCommand(c *cli.Context) error {
	if c.NArg() < 1 {
		return cliutil.UsageError(`"netscale tunnel cleanup" requires at least 1 argument, the IDs of the tunnels to cleanup connections.`)
	}
...
func buildTokenCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText:          "netscale tunnel [tunnel command options] token [subcommand options] TUNNEL",
		Description:        "netscale tunnel token will fetch the credentials token for a given tunnel (by its name or UUID), which is then used to run the tunnel. This command fails if the tunnel does not exist or has been deleted. Use the flag `netscale tunnel token --cred-file /my/path/file.json TUNNEL` to output the token to the credentials JSON file. Note: this command only works for Tunnels created since netscale version 2022.3.0",
...
func tokenCommand(c *cli.Context) error {
	...
	if c.NArg() != 1 {
		return cliutil.UsageError(`"netscale tunnel token" requires exactly 1 argument, the name or UUID of tunnel to fetch the credentials token for.`)
	}
	tunnelID, err := sc.findID(c.Args().First())
	if err != nil {
		return errors.Wrap(err, "error parsing tunnel ID")
...
func buildRouteCommand() *cli.Command {
	return &cli.Command{
		Name:      "route",
		Usage:     "Define which traffic routed from Khulnasoft edge to this tunnel: requests to a DNS hostname, to a Khulnasoft Load Balancer, or traffic originating from Khulnasoft WARP clients",
		UsageText: "netscale tunnel [tunnel command options] route [subcommand options] [dns TUNNEL HOSTNAME]|[lb TUNNEL HOSTNAME LB-POOL]|[ip NETWORK TUNNEL]",
		Description: `The route command defines how Khulnasoft will proxy requests to this tunnel.

To route a hostname by creating a DNS CNAME record to a tunnel:
   netscale tunnel route dns <tunnel ID or name> <hostname>
You can read more at: https://developers.khulnasoft.com/khulnasoft-one/connections/connect-apps/routing-to-tunnel/dns

To use this tunnel as a load balancer origin, creating pool and load balancer if necessary:
   netscale tunnel route lb <tunnel ID or name> <hostname> <load balancer pool>
You can read more at: https://developers.khulnasoft.com/khulnasoft-one/connections/connect-apps/routing-to-tunnel/lb

For Khulnasoft WARP traffic to be routed to your private network, reachable from this tunnel as origins, use:
   netscale tunnel route ip <network CIDR> <tunnel ID or name>
Further information about managing Khulnasoft WARP traffic to your tunnel is available at:
   netscale tunnel route ip --help
`,
		CustomHelpTemplate: commandHelpTemplate(),
		Subcommands: []*cli.Command{
			{
				...
				UsageText:   "netscale tunnel route dns [TUNNEL] [HOSTNAME]",
			...
			},
			{
				...
				UsageText:   "netscale tunnel route lb [TUNNEL] [HOSTNAME] [LB-POOL-NAME]",
...
func routeDnsCommand(c *cli.Context) error {
	if c.NArg() != 2 {
		return cliutil.UsageError(`This command expects the format "netscale tunnel route dns <tunnel name/id> <hostname>"`)
	}
...
func routeLbCommand(c *cli.Context) error {
	if c.NArg() != 3 {
		return cliutil.UsageError(`This command expects the format "netscale tunnel route lb <tunnel name/id> <hostname> <load balancer pool>"`)
	}
...
func commandHelpTemplate() string {
	...
	for _, f := range configureNetscaleFlags(false) {
