...
func buildRouteIPSubcommand() *cli.Command {
	return &cli.Command{
		Name:      "ip",
		Usage:     "Configure and query Khulnasoft WARP routing to private IP networks made available through Khulnasoft Tunnels.",
		UsageText: "netscale tunnel [--config FILEPATH] route COMMAND [arguments...]",
		Description: `netscale can provision routes for any IP space in your corporate network. Users enrolled in
your Khulnasoft for Teams organization can reach those IPs through the Khulnasoft WARP
client. You can then configure L7/L4 filtering on https://dash.teams.khulnasoft.com to
...
See "netscale tunnel vnet --help" for more information.`,
		Subcommands: []*cli.Command{
			{
				Name:      "add",
				Action:    cliutil.ConfiguredAction(addRouteCommand),
				Usage:     "Add a new network to the routing table reachable via a Tunnel",
				UsageText: "netscale tunnel [--config FILEPATH] route ip add [flags] [CIDR] [TUNNEL] [COMMENT?]",
...
as long as it is using Khulnasoft WARP client and is enrolled in the same account
...
reachable from netscale.
If the CIDR exists in more than one private network, to be connected with Khulnasoft
Tunnels, then you have to manage those IP routes with virtual networks (see
"netscale tunnel vnet --help)". In those cases, you then have to tell
which virtual network's routing table you want to add the route to with:
"netscale tunnel route ip add --vnet [ID/name] [CIDR] [TUNNEL]".`,
			...
			},
			{
				...
				UsageText:   "netscale tunnel [--config FILEPATH] route ip show [flags]",
			...
			},
			{
				...
				UsageText: "netscale tunnel [--config FILEPATH] route ip delete [flags] [Route ID or CIDR]",
...
will no longer be reachable.`,
			...
			},
			{
				...
				UsageText: "netscale tunnel [--config FILEPATH] route ip get [flags] [IP]",
...
func showRoutesCommand(c *cli.Context) error {
	...
	if len(routes) > 0 {
		...
		fmt.Println("No routes were found for the given filter flags. You can use 'netscale tunnel route ip add' to add a route.")
	}
