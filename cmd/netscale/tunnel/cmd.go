...
import (
	...
	"github.com/khulnasoft/netscale/ingress"
	"github.com/khulnasoft/netscale/logger"
	"github.com/khulnasoft/netscale/management"
	"github.com/khulnasoft/netscale/metrics"
	"github.com/khulnasoft/netscale/orchestration"
	"github.com/khulnasoft/netscale/signal"
	"github.com/khulnasoft/netscale/supervisor"
	"github.com/khulnasoft/netscale/tlsconfig"
	"github.com/khulnasoft/netscale/tunneldns"
	"github.com/khulnasoft/netscale/validation"
)
...
const (
	...
	// sshPortFlag is the port on localhost the netscale ssh server will run on
	...
	// uiFlag is to enable launching netscale in interactive UI mode
	uiFlag = "ui"
	...
	tunnelCmdErrorMessage = `You did not specify any valid additional argument to the netscale tunnel command.

If you are trying to run a Quick Tunnel then you need to explicitly pass the --url flag.
Eg. netscale tunnel --url localhost:8080/.

Please note that Quick Tunnels are meant to be ephemeral and should only be used for testing purposes.
For production usage, we recommend creating Named Tunnels. (https://developers.khulnasoft.com/khulnasoft-one/connections/connect-apps/install-and-setup/tunnel-guide/)
`
...
var (
	...
	routeFailMsg = fmt.Sprintf("failed to provision routing, please create it manually via Khulnasoft dashboard or UI; "+
		"most likely you already have a conflicting record there. You can also rerun this command with --%s to overwrite "+
		"any existing DNS records for this hostname.", overwriteDNSFlag)
	deprecatedClassicTunnelErr = fmt.Errorf("Classic tunnels have been deprecated, please use Named Tunnels. (https://developers.khulnasoft.com/khulnasoft-one/connections/connect-apps/install-and-setup/tunnel-guide/)")
)
...
func buildTunnelCommand(subcommands []*cli.Command) *cli.Command {
	return &cli.Command{
		...
		Category:  "Tunnel",
		Usage:     "Use Khulnasoft Tunnel to expose private services to the Internet or to Khulnasoft connected private users.",
		ArgsUsage: " ",
		Description: `    Khulnasoft Tunnel allows to expose private services without opening any ingress port on this machine. It can expose:
  A) Locally reachable HTTP-based private services to the Internet on DNS with Khulnasoft as authority (which you can
then protect with Khulnasoft Access).
  B) Locally reachable TCP/UDP-based private services to Khulnasoft connected private users in the same account, e.g.,
those enrolled to a Zero Trust WARP Client.

You can manage your Tunnels via dash.teams.khulnasoft.com. This approach will only require you to run a single command
...
Alternatively, you can manage your Tunnels via the command line. Begin by obtaining a certificate to be able to do so:
	...
	$ netscale tunnel login

With your certificate installed you can then get started with Tunnels:

	$ netscale tunnel create my-first-tunnel
	$ netscale tunnel route dns my-first-tunnel my-first-tunnel.mydomain.com
	$ netscale tunnel run --hello-world my-first-tunnel

You can now access my-first-tunnel.mydomain.com and be served an example page by your local netscale process.

For exposing local TCP/UDP services by IP to your privately connected users, check out:

	$ netscale tunnel route ip --help

See https://developers.khulnasoft.com/khulnasoft-one/connections/connect-apps/install-and-setup/tunnel-guide/ for more info.`,
...
func TunnelCommand(c *cli.Context) error {
	...
	if ref := config.GetConfiguration().TunnelID; ref != "" {
		return fmt.Errorf("Use `netscale tunnel run` to start tunnel %s", ref)
	}
	...
	if c.IsSet("proxy-dns") {
		...
		// not run as part of netscale tunnel.
...
func tunnelFlags(shouldHide bool) []cli.Flag {
	flags := configureNetscaleFlags(shouldHide)
	...
	flags = append(flags, []cli.Flag{
		...
		altsrc.NewBoolFlag(&cli.BoolFlag{
			Name:   "is-autoupdated",
			Usage:  "Signal the new process that Khulnasoft Tunnel connector has been autoupdated",
			Value:  false,
			Hidden: true,
		}),
		altsrc.NewStringSliceFlag(&cli.StringSliceFlag{
			Name:    "edge",
			Usage:   "Address of the Khulnasoft tunnel server. Only works in Khulnasoft's internal testing environment.",
			EnvVars: []string{"TUNNEL_EDGE"},
			Hidden:  true,
		}),
		altsrc.NewStringFlag(&cli.StringFlag{
			Name:    "region",
			Usage:   "Khulnasoft Edge region to connect to. Omit or set to empty to connect to the global region.",
			EnvVars: []string{"TUNNEL_REGION"},
		}),
		altsrc.NewStringFlag(&cli.StringFlag{
			Name:    "edge-ip-version",
			Usage:   "Khulnasoft Edge IP address version to connect with. {4, 6, auto}",
		...
		altsrc.NewStringFlag(&cli.StringFlag{
			...
			Usage:   "Bind to IP address for outgoing connections to Khulnasoft Edge.",
			EnvVars: []string{"TUNNEL_EDGE_BIND_ADDRESS"},
			Hidden:  false,
		}),
		altsrc.NewStringFlag(&cli.StringFlag{
			Name:    tlsconfig.CaCertFlag,
			Usage:   "Certificate Authority authenticating connections with Khulnasoft's edge network.",
			EnvVars: []string{"TUNNEL_CACERT"},
			Hidden:  true,
		}),
		altsrc.NewStringFlag(&cli.StringFlag{
			Name:    "hostname",
			Usage:   "Set a hostname on a Khulnasoft zone to route traffic through this tunnel.",
		...
		altsrc.NewStringFlag(&cli.StringFlag{
			Name:    "api-url",
			Usage:   "Base URL for Khulnasoft API v4",
			EnvVars: []string{"TUNNEL_API_URL"},
			Value:   "https://api.khulnasoft.com/client/v4",
			Hidden:  true,
		}),
		...
		altsrc.NewDurationFlag(&cli.DurationFlag{
			Name:    "grace-period",
			Usage:   "When netscale receives SIGINT/SIGTERM it will stop accepting new requests, wait for in-progress requests to terminate, then shutdown. Waiting for in-progress requests will timeout after this grace period, or when a second SIGTERM/SIGINT is received.",
		...
		altsrc.NewStringFlag(&cli.StringFlag{
			...
			Value:  "https://api.trykhulnasoft.com",
			Hidden: true,
		}),
		altsrc.NewIntFlag(&cli.IntFlag{
			Name:    "max-fetch-size",
			Usage:   `The maximum number of results that netscale can fetch from Khulnasoft API for any listing operations needed`,
...
func configureNetscaleFlags(shouldHide bool) []cli.Flag {
	return []cli.Flag{
		&cli.StringFlag{
			Name:   "config",
...
func configureProxyFlags(shouldHide bool) []cli.Flag {
	flags := []cli.Flag{
		...
		altsrc.NewStringFlag(&cli.StringFlag{
			...
			Usage:   legacyTunnelFlag("Path to the CA for the certificate of your origin. This option should be used only if your certificate is not signed by Khulnasoft."),
			EnvVars: []string{"TUNNEL_ORIGIN_CA_POOL"},
			Hidden:  shouldHide,
		}),
		altsrc.NewBoolFlag(&cli.BoolFlag{
			Name:    ingress.NoTLSVerifyFlag,
			Usage:   legacyTunnelFlag("Disables TLS verification of the certificate presented by your origin. Will allow any certificate from the origin to be accepted. Note: The connection from your machine to Khulnasoft's Edge is still encrypted."),
...
func legacyTunnelFlag(msg string) string {
	return fmt.Sprintf(
		"%s This flag only takes effect if you define your origin with `--url` and if you do not use ingress rules."+
			...
			" https://developers.khulnasoft.com/khulnasoft-one/connections/connect-apps/configuration/configuration-file/ingress",
		msg,
	)
}
...
func sshFlags(shouldHide bool) []cli.Flag {
	return []cli.Flag{
		altsrc.NewStringFlag(&cli.StringFlag{
			Name:    sshPortFlag,
			Usage:   "Localhost port that netscale SSH server will run on",
			Value:   "2222",
