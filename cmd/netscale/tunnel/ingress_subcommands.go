...
func buildIngressSubcommand() *cli.Command {
	return &cli.Command{
		Name:      "ingress",
		Category:  "Tunnel",
		Usage:     "Validate and test netscale tunnel's ingress configuration",
		UsageText: "netscale tunnel [--config FILEPATH] ingress COMMAND [arguments...]",
		Hidden:    true,
		Description: ` Netscale lets you route traffic from the internet to multiple different addresses on your
		...
		To ensure netscale can route all incoming requests, the last rule must be a catch-all
...
func buildValidateIngressCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText:   "netscale tunnel [--config FILEPATH] ingress validate",
...
func buildTestURLCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText: "netscale tunnel [--config FILEPATH] ingress rule URL",
		ArgsUsage: "URL",
...
// validateIngressCommand check the syntax of the ingress rules in the netscale config file
...
func testURLCommand(c *cli.Context) error {
	...
	if requestArg == "" {
		return errors.New("netscale tunnel rule expects a single argument, the URL to test")
	}
