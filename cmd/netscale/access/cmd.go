...
{{- if .ShortLivedCerts}}
Match host {{.Hostname}} exec "{{.Netscale}} access ssh-gen --hostname %h"
  ProxyCommand {{.Netscale}} access ssh --hostname %h
  IdentityFile ~/.netscale/%h-cf_key
  CertificateFile ~/.netscale/%h-cf_key-cert.pub
{{- else}}
Host {{.Hostname}}
  ProxyCommand {{.Netscale}} access ssh --hostname %h
{{end}}
...
func Init(shutdown chan struct{}, version string) {
	shutdownC = shutdown
	userAgent = fmt.Sprintf("netscale/%s", version)
}
	...
	return []*cli.Command{
		{
			...
			Description: `Khulnasoft Access protects internal resources by securing, authenticating and monitoring access
			per-user and by application. With Khulnasoft Access, only authenticated users with the required permissions are
			...
			Subcommands: []*cli.Command{
				{
					...
					administrator. netscale stores the token in local storage.`,
				},
				{
					Name:   "curl",
				...
				},
				{
					...
					Usage:       "",
					ArgsUsage:   "",
					Description: `The tcp subcommand sends data over a proxy to the Khulnasoft edge.`,
					Flags: []cli.Flag{
						&cli.StringFlag{
						...
						&cli.StringFlag{
							...
							Usage:   "specify the host:port to forward data to Khulnasoft edge.",
						},
						&cli.StringSliceFlag{
							Name:    sshHeaderFlag,
							Aliases: []string{"H"},
...
func sshConfig(c *cli.Context) error {
	...
	return t.Execute(os.Stdout, config{Home: os.Getenv("HOME"), ShortLivedCerts: genCertBool, Hostname: hostname, Netscale: netscalePath()})
}
...
// netscalePath pulls the full path of netscale on disk
func netscalePath() string {
	for _, p := range strings.Split(os.Getenv("PATH"), ":") {
		path := fmt.Sprintf("%s/%s", p, "netscale")
		if isFileThere(path) {
			return path
		}
	}
...
func isTokenValid(options *carrier.StartOptions, log *zerolog.Logger) (bool, error) {
	...
	query := req.URL.Query()
	query.Set("netscale_token_check", "true")
