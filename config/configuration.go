...
var (
	...
	DefaultUnixConfigLocation = "/usr/local/etc/netscale"

	// DefaultUnixLogLocation is the primary location to find log files
	DefaultUnixLogLocation = "/var/log/netscale"

	// Launchd doesn't set root env variables, so there is default
	// Windows default config dir was ~/khulnasoft-warp in documentation; let's keep it compatible
	defaultUserConfigDirs = []string{"~/.netscale", "~/.khulnasoft-warp", "~/khulnasoft-warp"}
	defaultNixConfigDirs  = []string{"/etc/netscale", DefaultUnixConfigLocation}
...
func DefaultConfigDirectory() string {
	if runtime.GOOS == "windows" {
		...
		if path == "" {
			path = filepath.Join(os.Getenv("ProgramFiles(x86)"), "netscale")
...
// customize how netscale sends requests to origin services. It is used to set
