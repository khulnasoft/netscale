...
import (
	...
	"github.com/khulnasoft/netscale/edgediscovery/allregions"
	"github.com/khulnasoft/netscale/features"
	"github.com/khulnasoft/netscale/h2mux"
	...
	quicpogs "github.com/khulnasoft/netscale/quic"
	"github.com/khulnasoft/netscale/retry"
...
func (e *EdgeTunnelServer) Serve(ctx context.Context, connIndex uint8, protocolFallback *protocolFallback, connectedSignal *signal.Signal) error {
	...
	logger := e.config.Log.With().
		Int(management.EventTypeKey, int(management.Netscale)).
...
func selectNextProtocol(
	...
	if protocolBackoff.ReachedMaxRetries() || (hasFallback && isQuicBroken) {
		if isQuicBroken {
			connLog.Warn().Msg("If this log occurs persistently, and netscale is unable to connect to " +
				"Khulnasoft Network with `quic` protocol, then most likely your machine/network is getting its egress " +
				"UDP to port 7844 (or others) blocked or dropped. Make sure to allow egress connectivity as per " +
				"https://developers.khulnasoft.com/khulnasoft-one/connections/connect-apps/configuration/ports-and-ips/\n" +
				"If you are using private routing to this Tunnel, then ICMP, UDP (and Private DNS Resolution) will not work " +
				"unless your netscale can connect with Khulnasoft Network with `quic`.")
		}
...
func (e *EdgeTunnelServer) serveConnection(
	...
	case connection.HTTP2:
		...
		if err != nil {
			connLog.ConnAwareLogger().Err(err).Msg("Unable to establish connection with Khulnasoft edge")
			return err, true
		}
...
func activeIncidentsMsg(incidents []Incident) string {
	preamble := "There is an active Khulnasoft incident that may be related:"
	if len(incidents) > 1 {
		preamble = "There are active Khulnasoft incidents that may be related:"
	}
