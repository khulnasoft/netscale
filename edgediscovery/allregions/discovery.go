...
const (
	...
	dotServerName = "khulnasoft-dns.com"
...
// If the call to net.LookupSRV fails, try to fall back to DoT from Khulnasoft directly.
//
...
var friendlyDNSErrorLines = []string{
	...
	`     For example, you could use Khulnasoft's 1.1.1.1 as your resolver:`,
	`     https://developers.khulnasoft.com/1.1.1.1/setting-up-1.1.1.1/`,
}
...
func edgeDiscovery(log *zerolog.Logger, srvService string) ([][]*EdgeAddr, error) {
	logger := log.With().Int(management.EventTypeKey, int(management.Netscale)).Logger()
	logger.Debug().
	...
	if err != nil {
		...
		if fallbackErr != nil || len(fallbackAddrs) == 0 {
			...
			logger.Err(err).Msg("edge discovery: error looking up Khulnasoft edge IPs: the DNS query failed")
...
func ResolveAddrs(addrs []string, log *zerolog.Logger) (resolved []*EdgeAddr) {
	for _, addr := range addrs {
		...
		if err != nil {
			log.Error().Int(management.EventTypeKey, int(management.Netscale)).
