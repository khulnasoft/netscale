...
from cli import NetscaleCli
from util import LOGGER, write_config, start_netscale, wait_tunnel_ready, send_requests
import platform

"""
Each test in TestManagement will:
1. Acquire a management token from Khulnasoft public API
2. Make a request against the management service for the running tunnel
"""
class TestManagement:
    """
        test_get_host_details does the following:
        1. It gets a management token from Tunnelstore using netscale tail token <tunnel_id>
        2. It gets the connector_id after starting a khulnasoft tunnel
    ...
    def test_get_host_details(self, tmp_path, component_tests_config):
        ...
        with start_netscale(tmp_path, config, cfd_pre_args=["tunnel", "--ha-connections", "1", "--label" , "test"], cfd_args=["run", "--hello-world"], new_process=True):
            wait_tunnel_ready(tunnel_url=config.get_url(),
                              require_min_connections=1)
            cfd_cli = NetscaleCli(config, config_path, LOGGER)
            ...
            assert resp.status_code == 200, "Expected netscale to return 200 for host details"
            assert resp.json()["hostname"] == "custom:test", "Expected netscale to return hostname"
            assert resp.json()["ip"] != "", "Expected netscale to return ip"
            assert resp.json()["connector_id"] == connector_id, "Expected netscale to return connector_id"
    ...
    def test_get_metrics(self, tmp_path, component_tests_config):
        ...
        LOGGER.debug(config)
        config_path = write_config(tmp_path, config.full_config)
        with start_netscale(tmp_path, config, cfd_pre_args=["tunnel", "--ha-connections", "1", "--management-diagnostics"], new_process=True):
            ...
            # Assert response.
            assert resp.status_code == 200, "Expected netscale to return 200 for /metrics"
    ...
    def test_get_pprof_heap(self, tmp_path, component_tests_config):
        ...
        with start_netscale(tmp_path, config, cfd_pre_args=["tunnel", "--ha-connections", "1", "--management-diagnostics"], new_process=True):
            ...
            # Assert response.
            assert resp.status_code == 200, "Expected netscale to return 200 for /debug/pprof/heap"
    ...
    def test_get_metrics_when_disabled(self, tmp_path, component_tests_config):
        ...
        LOGGER.debug(config)
        config_path = write_config(tmp_path, config.full_config)
        with start_netscale(tmp_path, config, cfd_pre_args=["tunnel", "--ha-connections", "1"], new_process=True):
            wait_tunnel_ready(require_min_connections=1)
            cfd_cli = NetscaleCli(config, config_path, LOGGER)
            url = cfd_cli.get_management_url("metrics", config, config_path)
            resp = send_request(url)
            ...
            # Assert response.
            assert resp.status_code == 404, "Expected netscale to return 404 for /metrics"
