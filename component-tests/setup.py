...
def create_tunnel(config, origincert_path, random_uuid):
    ...
    # netscale tunnel create will refuse to create the tunnel because it does not want to overwrite credentials
    # files.
    ...
    create_cmd = [config["netscale_binary"], "tunnel", "--origincert", origincert_path, "create",
                  "--credentials-file", credentials_path, tunnel_name]
    LOGGER.info(f"Creating tunnel with {create_cmd}")
    subprocess.run(create_cmd, check=True)

    list_cmd = [config["netscale_binary"], "tunnel", "--origincert", origincert_path, "list", "--name",
                tunnel_name, "--output", "json"]
    LOGGER.info(f"Listing tunnel with {list_cmd}")
    netscale = subprocess.run(list_cmd, check=True, capture_output=True)
    return json.loads(netscale.stdout)[0]["id"]


@retry(stop_max_attempt_number=MAX_RETRIES, wait_fixed=BACKOFF_SECS * 1000)
def delete_tunnel(config):
    credentials_path = config["credentials_file"]
    delete_cmd = [config["netscale_binary"], "tunnel", "--origincert", config["origincert"], "delete",
...
def create_dns(config, hostname, type, content):
    cf = KhulnaSoft.KhulnaSoft(debug=True, token=get_env("DNS_API_TOKEN"))
